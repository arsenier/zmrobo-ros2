...
sensor_msgs::msg::Imu Mpu6050;//Instantiate an IMU object 
...
void turn_on_robot::Akm_cmd_Vel_Callback(const ackermann_msgs::msg::AckermannDriveStamped::SharedPtr akm_ctl)
{
  ...
  {
    RCLCPP_ERROR(node->get_logger(), "Unable to send data through serial port");
  }
}
/**************************************
Date: January 28, 2021
...
void turn_on_robot::Cmd_Vel_Callback(const geometry_msgs::msg::Twist::SharedPtr twist_aux)
{
  ...
  // RCLCPP_INFO(node->get_logger(), "Cmd vel callback");
...
{
  sensor_msgs::msg::Imu Imu_Data_Pub; //Instantiate IMU topic data
  Imu_Data_Pub.header.stamp = node->now(); 
...
void turn_on_robot::Publish_Odom()
{
    ...
    nav_msgs::msg::Odometry odom; //Instance the odometer topic data
...
{
    std_msgs::msg::Float32 voltage_msgs; //Define the data type of the power supply voltage publishing topic
...
void turn_on_robot::Control()
{
  ...
  while(rclcpp::ok())
  {
    ...
    rclcpp::spin_some(node);   //The loop waits for the callback function //循环等待回调函数
    }
}
/**************************************
Date: January 28, 2021
...
功能: 构造函数, 只执行一次，用于初始化
***************************************/
turn_on_robot::turn_on_robot(const rclcpp::Node::SharedPtr & node_ptr):Sampling_Time(0),Power_voltage(0)
{
  node = node_ptr;
  //Clear the data
  //清空数据
  ...
  node->declare_parameter<std::string>("usart_port_name", "/dev/ttyACM1");
  ...
  Cmd_Vel_Sub     = node->create_subscription<geometry_msgs::msg::Twist>(cmd_vel,  rclcpp::QoS(100), std::bind(&turn_on_robot::Cmd_Vel_Callback, this, _1));
  Akm_cmd_Vel_Sub = node->create_subscription<ackermann_msgs::msg::AckermannDriveStamped>(akm_cmd_vel, rclcpp::QoS(100), std::bind(&turn_on_robot::Akm_cmd_Vel_Callback, this, _1));

  RCLCPP_INFO(node->get_logger(), "Data ready"); //Prompt message //提示信息
  ...
  {
    RCLCPP_ERROR(node->get_logger(), "zmrobo can not open serial port,Please check the serial port cable! "); //If opening the serial port fails, an error message is printed //如果开启串口失败，打印错误信息
  }
  if(Stm32_Serial.isOpen())
  {
    RCLCPP_INFO(node->get_logger(), "zmrobo serial port opened"); //Serial port opened successfully //串口开启成功提示
  }
}
/**************************************
Date: January 28, 2021
...
turn_on_robot::~turn_on_robot()
{
  ...
  {
    RCLCPP_ERROR(node->get_logger(), "Unable to send data through serial port"); //If sending data fails, an error message is printed //如果发送数据失败,打印错误信息
  }
  Stm32_Serial.close(); //Close the serial port //关闭串口  
  RCLCPP_INFO(node->get_logger(), "Shutting down"); //Prompt message //提示信息
}
